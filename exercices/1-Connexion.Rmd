...
Avec l'interface graphique, il suffit de naviguer vers <http://iml-science-1.ent.dfo-mpo.ca:8888/search?t=projects> pour voir les projets accessible par ton utilisateur.

Nous pouvons faire la même chose en utilisant l'API au lien suivant (essayez le!) <http://iml-science-1.ent.dfo-mpo.ca:8888/api/v1/projects>.

Le lien est composé de deux parties **l'hôte** (`http://iml-science-1.ent.dfo-mpo.ca:8888/`) suivi du **chemin** (`api/v1/projects`).

Nous allons utiliser l'hôte du déploiement BIIGLE au MPO (`http://iml-science-1.ent.dfo-mpo.ca:8888/`), mais il est possible de prendre le déploiement officiel `biigle.de`. Le chemin (`api/v1/projects`) détermine la ressource qui est concerné, dans l'exemple nous voulons obtenir la liste de projets en utilisant la version 1 de l'API.
...
reponse <- GET(
  url = "http://iml-science-1.ent.dfo-mpo.ca:8888/api/v1/projects/",
  accept_json()
  )
```
